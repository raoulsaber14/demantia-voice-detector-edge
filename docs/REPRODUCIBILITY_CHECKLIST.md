...
- [ ] Demo output contract: the committed demos are research/demo review interfaces. The Flask app shows risk band, risk score, screening indicator, confidence, and per-class probabilities; the Streamlit app shows predicted label, confidence, dementia probability, and threshold controls. Source: `docs/DEMO_ALIGNMENT_SPEC_FINAL.md`; `app/flask/templates/index.html`; `app/streamlit/demo_app.py`.
