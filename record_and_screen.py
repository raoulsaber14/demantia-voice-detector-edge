...
Usage:
    ...
    python record_and_screen.py

    # Real model (after teammate ships ONNX):
    python record_and_screen.py --config edge_inference/model_config.yaml

    # Shorter recording:
    python record_and_screen.py --duration 10

    # Use a specific USB mic (run --list-devices to find it):
    python record_and_screen.py --device 2

    # Process an existing WAV instead of recording:
    python record_and_screen.py --input some_recording.wav

    # Run a quick latency benchmark after the inference:
    python record_and_screen.py --benchmark
...
def main():
    ...
    parser.add_argument(
        "--config", type=Path, default=Path("edge_inference/model_config.yaml"),
        help="Path to the inference config YAML (default: edge_inference/model_config.yaml)",
    )
    ...
    if not args.config.exists():
        ...
        logger.error("Hint: from the project root, default is edge_inference/model_config.yaml")
        return 1
