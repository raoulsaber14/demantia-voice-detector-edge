...
Usage:
    ...
    # Custom duration
    python -m scripts.pipeline_diagnostic --duration 10

    # Process an existing WAV file instead of recording
    python -m scripts.pipeline_diagnostic --input existing_recording.wav

    # List available audio devices (useful for finding USB mic index)
    python -m scripts.pipeline_diagnostic --list-devices

    # Use a specific device index
    python -m scripts.pipeline_diagnostic --device 2

    # Adjust noise reduction aggressiveness (1.0 = mild, 4.0 = aggressive)
    python -m scripts.pipeline_diagnostic --noise-alpha 3.0

    # Skip specific stages for debugging; VAD is skipped by default to preserve long pauses
    python -m scripts.pipeline_diagnostic --enable-vad  # enable VAD trimming for diagnostics
    python -m scripts.pipeline_diagnostic --skip-denoise
